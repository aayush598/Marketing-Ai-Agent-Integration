...
def generate_social_media_post_from_structure(post_structure):
    ...
    prompt = f"Generate a detailed social media post based on the following structured details with no extra stars and do not use any special characters,only use characters and hashtag and also do not use emojis, do add any stars or emojis, content size and format must be same as the platform provided:\n{post_structure}"
    response = gemini_text_model.generate_content(prompt)
    return response.text
